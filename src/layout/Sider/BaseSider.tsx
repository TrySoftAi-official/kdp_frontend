...
export const Sidebar: React.FC = () => {
  ...
  const canAccessRoute = (item: NavItem) => {
    ...
    if (item.roles && (!user || !user.role || !item.roles.includes(user.role))) {
      // Special case: Organization features for Enterprise users regardless of role
      if (item.href === '/organization' && isEnterpriseUser) {
        return true;
      }
      return false;
    }
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1 mt-1">
                      ...
                      <Badge 
                        variant="outline" 
                        className={cn("text-xs font-medium", getRoleColor(user.role || 'guest'))}
                      >
                        {user.role || 'guest'}
                      </Badge>
                    </div>
